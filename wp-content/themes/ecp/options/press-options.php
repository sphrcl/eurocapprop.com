...
$press_box = array(
	...
	'fields' => array(
 		...
 		array( 
			"name" => "Upload",
			"desc" => "Upload press title image file here.",
			"id" => $prefix."_press_image",
			"type" => "upload",
			"std" => ""
        )
...
add_action('admin_menu', 'press_box_mythemes_add_boxel');
// Add meta boxel
function press_box_mythemes_add_boxel() {
	global $press_box;
	foreach ( array( 'press' ) as $page )
	add_meta_box($press_box['id'], $press_box['title'], 'press_box_mythemes_show_boxel', $page, $press_box['context'], 			$press_box['priority']);
}
// Callback function to show fields in meta boxel
function press_box_mythemes_show_boxel() {
...
jQuery(document).ready(function() {
	...
	echo '<input type="hidden" name="press_box_mythemes_press_box_nonce" value="', wp_create_nonce(basename(__FILE__)), '" />';
...
add_action('save_post', 'press_box_mythemes_save_data');
// Save data from meta boxel
function press_box_mythemes_save_data($post_id) {
	global $press_box;	
	// verify nonce
	if (!wp_verify_nonce($_POST['press_box_mythemes_press_box_nonce'], basename(__FILE__))) {
		return $post_id;
	}
	// check autosave
